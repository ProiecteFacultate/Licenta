                                              ...
                                              char* directoryPath, ExtentsDirectoryRecord** extentsDirectoryRecord, uint32_t& nodeOfNewRecord, uint32_t extentOverflowIndex)
{
    ...
    if(strcmp(actualDirectoryName, "Root\0") != 0)
        return EOF_SEARCH_RECORD_BY_FULL_PATH_PARENT_DO_NOT_EXIST;

    *extentsDirectoryRecord = nullptr;
    ExtentsDirectoryRecord* searchedExtentsDirectoryRecord = new ExtentsDirectoryRecord();

    while(actualDirectoryName != nullptr)
    {
        uint32_t searchedExtentsDirectoryRecordResult = eof_searchDirectoryRecordByDirectoryNameBeingGivenParentDirectoryRecord(diskInfo, volumeHeader, extentsFileHeaderNode,
                                                                                                                            *extentsDirectoryRecord,
                                                                                                                            actualDirectoryName,
                                                                                                                            searchedExtentsDirectoryRecord,
                                                                                                                            nodeOfNewRecord, extentOverflowIndex);

        if(searchedExtentsDirectoryRecordResult == EOF_SEARCH_DIRECTORY_RECORD_BY_DIRECTORY_NAME_BEING_GIVEN_PARENT_RECORD_SUCCESS)
            *extentsDirectoryRecord = searchedExtentsDirectoryRecord;
        else
        {
            delete searchedExtentsDirectoryRecord;

            if(searchedExtentsDirectoryRecordResult == EOF_SEARCH_DIRECTORY_RECORD_BY_DIRECTORY_NAME_BEING_GIVEN_PARENT_RECORD_KEY_DO_NOT_EXIT_IN_TREE)
                return EOF_SEARCH_RECORD_BY_FULL_PATH_PARENT_DO_NOT_EXIST;

            return EOF_SEARCH_RECORD_BY_FULL_PATH_FAILED_FOR_OTHER_REASON;
        }

        actualDirectoryName = strtok(nullptr, "/");
    }

    return EOF_SEARCH_RECORD_BY_FULL_PATH_SUCCESS;
}
                                                                             ...
                                                                             uint32_t extentOverflowIndex)
{
    if(parentDirectoryRecord == nullptr) //if we are looking for root, which is not an actual directory
    {
        ...
        return EOF_SEARCH_DIRECTORY_RECORD_BY_DIRECTORY_NAME_BEING_GIVEN_PARENT_RECORD_SUCCESS;
    }

    uint32_t actualNodeNumber = extentsFileHeaderNode->headerRecord.rootNode;
    char* nodeBuffer = new char[getExtentsOverflowFileNodeSize()];

    uint32_t readNodeFromDiskResult = eof_readNodeFromDisk(diskInfo, volumeHeader, nodeBuffer, actualNodeNumber);
    if(readNodeFromDiskResult == EOF_READ_NODE_FROM_DISK_FAILED)
    {
        delete[] nodeBuffer;
        return EOF_SEARCH_DIRECTORY_RECORD_BY_DIRECTORY_NAME_BEING_GIVEN_PARENT_RECORD_FAILED_FOR_OTHER_REASON;
    }

    ExtentsFileCatalogKey* searchedKey = new ExtentsFileCatalogKey();
    ...
    searchedKey->extentOverflowIndex = extentOverflowIndex;

    uint32_t searchRecordResult = eof_searchRecordForGivenNodeDataAndSearchedKey(diskInfo, volumeHeader, searchedKey, nodeBuffer, searchedDirectoryRecord,
                                                                             nodeNumberForRecord);

    switch (searchRecordResult) {
        case EOF_SEARCH_RECORD_IN_GIVEN_DATA_KEY_DO_NOT_EXIST_IN_TREE:
            return EOF_SEARCH_DIRECTORY_RECORD_BY_DIRECTORY_NAME_BEING_GIVEN_PARENT_RECORD_KEY_DO_NOT_EXIT_IN_TREE;
        case EOF_SEARCH_RECORD_IN_GIVEN_DATA_FAILED_FOR_OTHER_REASON:
            return EOF_SEARCH_DIRECTORY_RECORD_BY_DIRECTORY_NAME_BEING_GIVEN_PARENT_RECORD_FAILED_FOR_OTHER_REASON;
        default: //success
            return EOF_SEARCH_DIRECTORY_RECORD_BY_DIRECTORY_NAME_BEING_GIVEN_PARENT_RECORD_SUCCESS;
    }
