...
void commandListSubdirectories(DiskInfo* diskInfo, HFSPlusVolumeHeader* volumeHeader, CatalogFileHeaderNode* catalogFileHeaderNode, std::vector<std::string> commandTokens)
{
    if(commandTokens.size() >= 2)
    {
        if(commandTokens[1] == "-l")
            commandListSubdirectoriesWithSize(diskInfo, volumeHeader, catalogFileHeaderNode, commandTokens);
        else
...
void commandShowDirectoryAttributes(DiskInfo* diskInfo, HFSPlusVolumeHeader* volumeHeader, CatalogFileHeaderNode* catalogFileHeaderNode, std::vector<std::string> commandTokens)
{
    ...
    switch (getDirectoryDisplayableAttributesResult) {
        case DIRECTORY_GET_DISPLAYABLE_ATTRIBUTES_FAILED_GIVEN_DIRECTORY_DO_NOT_EXIST_OR_SEARCH_ERROR:
        ...
        case DIRECTORY_GET_DISPLAYABLE_ATTRIBUTES_SUCCESS:
            ...
            std::cout << "Created - " << attributes->CreatedYear << ":" << attributes->CreatedMonth << ":" << attributes->CreatedDay
                      << " - " << attributes->CreatedHour << ":" << attributes->CreatedMinute << ":" << attributes->CreatedSecond << '\n';
    ...
    switch (getSubdirectoriesResult) {
        ...
        case GET_SUBDIRECTORIES_SUCCESS:
            ...
            for(CatalogDirectoryRecord* child: subDirectories)
            {
                ...
                std::cout << fileName << " -- " << child->catalogData.fileSize << "\n";
            }
    }
}
